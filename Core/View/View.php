...
class View
{
    ...
    {
        // on check ici si l'utilisateur est en session sinon on redirige vers la page de connexion
        ...
        // on importe le template _header
        if ($this->is_complete) {
            require self::PATH_PARTIALS . '_header.html.php';
        }

        // on inclue le fichier de la vue
        require $this->getRequirePath();

        // on importe le template _footer
        if ($this->is_complete) {
